...
def generate_weather_email(json_path):
    ...
    date_str = data.get("summary_of_current_conditions", {}).get(
        "date", get_ph_time().strftime("%B %d, %Y")
    )
    ...
    # Reuse the same dictionary we used for system/date
    current = data.get("summary_of_current_conditions", {})

    metro_manila = (
        normalize_text(current.get("metro_manila_email"))
        or flatten_dict(current.get("metro_manila_email"))
        or "Generally stable weather conditions."
    )

    dumaguete = (
        normalize_text(current.get("dumaguete_email"))
        or flatten_dict(current.get("dumaguete_email"))
        or "Generally stable weather conditions."
    )
    ...
    <body style="font-family: Arial; font-size: 13px;">
        ...
        <p>
            Please find the weather summary for today, <b>{date_str}</b>, detailing current conditions 
            for Manila and Dumaguete based on the latest regional advisories.
        </p>
        ...
        <p>
            For real-time updates and interactive monitoring, you may access the 
            <a href="https://script.google.com/a/macros/ececontactcenters.com/s/AKfycbyb-WPlhrGMJH-xaTs8dFCdL2Hd57ny5DslnLeuzH-ZPrytn6-y_m0sIMxyJ999ZZvKdQ/exec" style="color: #2563eb; font-weight: bold; text-decoration: none;">
                Project AWRA Live WebApp
            </a>. 
            This dashboard provides 24/7 visibility into weather systems, site impact levels, and operational recommendations.
        </p>

        <p>We will continue to monitor the situation closely and provide additional updates as necessary.</p>
    ...
    pdf_filename = "weather_report.pdf"
...
if TEST_MODE:
    ...
    cc_recipients = [
        ...
        "residenthomeworkforce@ececontactcenters.com",
        "workforcerta@ececontactcenters.com",
